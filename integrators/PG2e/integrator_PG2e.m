...
%     F_INT  function handle  F_int(U, V) -> N x 1 internal force.
%              Linear:    F_int = @(U,V) K*U;
%              Nonlinear: F_int = @(U,V) InternalForce(U,V);
...
zV      = op.zeta * V_T;                                  % zeta * V_T

V_Tpp   = op.dP \ (3*V_T - 2*dt*zV + 3*dt*RmF_T);         % predictor
U_Tp1   = U_T + 0.5*dt*(V_T + V_Tpp);                     % displacement

RmF_Tp1 = op.Msolve(R_Tp1 - F_int(U_Tp1, V_Tpp));         % r - f at t+dt
V_Tp1   = op.dQ \ (2*V_T - dt*zV + dt*(RmF_T + RmF_Tp1)); % corrector
